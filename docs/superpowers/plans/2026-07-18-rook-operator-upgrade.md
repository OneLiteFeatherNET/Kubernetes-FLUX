...
spec:
  interval: 5m
  url: https://ceph.github.io/ceph-csi-operator-charts
```
...
**Interfaces:**
- Consumes: `HelmRepository` `ceph-csi-operator` from Task 4; findings file from Task 3 (`## Resolved values` section supersedes the draft below)
...
- [ ] **Step 1: Read Task 3's findings file and reconcile it with the draft below**

Open `/tmp/claude-1000/-mnt-projects-oss-onelitefeather-Kubernetes-FLUX/6c578aeb-9556-4bf9-bae0-646f3f6bd6b8/scratchpad/ceph-csi-drivers-research.md`. If its `## Resolved values` section differs from the draft in Step 2 (chart version, field names, whether `cephConnections`/`clientProfiles` are needed, where `hostNetwork` belongs, whether a gRPC-metrics equivalent exists), use the findings file's version instead of the draft — the draft is only a pre-verified starting point.

- [ ] **Step 2: Create the new HelmRelease**
...
spec:
  ...
  chart:
    spec:
      ...
      version: "REPLACE_WITH_TASK_3_CHART_VERSION" # e.g. ">=X.Y.0 <X+1.0.0" — copy the exact floor from the "## Chart version" section of Task 3's findings file, do not guess
      sourceRef:
        kind: HelmRepository
        name: ceph-csi-operator
        namespace: flux-system
...
(The `version` field above is intentionally not filled in with a real number here — it must come from Task 3's actual `helm search repo` output, not be invented. Copy the exact floor from the "## Chart version" section of Task 3's findings file into a `">=X.Y.0 <X+1.0.0"` constraint, matching the style used for `rook-ceph` elsewhere in this repo, before committing. The per-container resource figures from the old `csi.csiRBDProvisionerResource`/`csiRBDPluginResource`/etc. blocks have been consolidated into the two pod-side blocks this chart's schema exposes; if Task 3 found real per-container overrides are supported instead, use those exact original numbers per-container rather than this consolidation.)

- [ ] **Step 3: Register the new HelmRelease**
...
- [ ] **Step 4: Bump the `rook-ceph` chart version to 1.20**
...
- [ ] **Step 5: Remove the obsolete `csi:` block**
...
- [ ] **Step 6: Render and verify both releases**
